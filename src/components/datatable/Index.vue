<template>
    ...
    <div class="datatable-header">
      <div class="datatable-left-header">
        <select :value="props.config.showPerPage" class="select-show-per-page" @change="changeShowPerPage" v-if="mode == 'v1'">
        ...
        <div class="d-flex" style="border: 1px #ddd solid; border-radius: 3px" v-if="mode == 'v2'">
          ...
          <form autocomplete="off" @submit.prevent>
            <input type="text" class="search-data" :value="props.config.search" @input="(evt) => { changeSearch(evt); getDataAfterSearch(); }" placeholder="Cari" />
          </form>
        </div>
      </div>
      <div class="datatable-right-header">
        <div class="d-flex" style="border: 1px #ddd solid; border-radius: 3px" v-if="mode == 'v1'">
          <div style="background: #fff; padding: 5px 15px">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
              <rect opacity="0.5" x="17.0365" y="15.1223" width="8.15546" height="2" rx="1" transform="rotate(45 17.0365 15.1223)" fill="currentColor"></rect>
              <path d="M11 19C6.55556 19 3 15.4444 3 11C3 6.55556 6.55556 3 11 3C15.4444 3 19 6.55556 19 11C19 15.4444 15.4444 19 11 19ZM11 5C7.53333 5 5 7.53333 5 11C5 14.4667 7.53333 17 11 17C14.4667 17 17 14.4667 17 11C17 7.53333 14.4667 5 11 5Z" fill="currentColor"></path>
    ...
    <div class="datatable-body">        
      <table class="custom-vue-datatable">
        <thead>
          <tr>
            <th v-for="(item, index) in props.config.column" :key="index" :class="item.class" @click="changeSortBy(item.sortBy)" :style="item.sortColumn ? { cursor: 'pointer' } : {}">
              {{ item.text }}
              <span v-if="item.sortColumn !== false">
                <template v-if="props.config.sortBy === item.sortBy">
                    <svg v-if="props.config.order === 'asc'" enable-background="new 0 0 26 15" height="13px" version="1.1"
                        viewBox="0 0 26 15" width="13px" xml:space="preserve" xmlns="http://www.w3.org/2000/svg"
                        xmlns:xlink="http://www.w3.org/1999/xlink">
                        <polygon fill="#231F20"
                            points="23.303,15.002 12.467,4.166 1.63,15.002 -0.454,12.918 12.467,-0.002 14.551,2.082 25.387,12.918" />
                    </svg>
                    <svg v-else enable-background="new 0 0 26 15" height="13px" id="Layer_1" version="1.1"
                        viewBox="0 0 26 15" width="13px" xml:space="preserve" xmlns="http://www.w3.org/2000/svg"
                        xmlns:xlink="http://www.w3.org/1999/xlink">
                        <polygon fill="#231F20"
    ...
    <div class="datatable-footer">
        <div className="datatable-footer-left">
            <template v-if="mode == 'v1'">
                Showing {{Math.min((props.config.currentPage - 1) * props.config.showPerPage + 1, props.config.totalData)}} to {{Math.min(props.config.currentPage * props.config.showPerPage, props.config.totalData)}}  of {{props.config.totalData}} records
            </template>
        ...
        <div class="datatable-footer-right">
            ...
            <li v-for="(item, index) in pages" :key="index">
                <button :class="{ active: props.config.currentPage === item.page }" @click="onClickPage(item.page)" :disabled="item.isDisabled">
                {{ item.page }}
                </button>
            </li>
