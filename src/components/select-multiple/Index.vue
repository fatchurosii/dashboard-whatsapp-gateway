<template>
    ...
    <div
      class="select-multiple-box"
      :style="{ background: disabled ? '#eff2f5' : '#fff' }"
      @click="handleDropdownToggle"
    >
      <div v-if="hasValue(modelValue)" class="select-multiple-selected-value-group">
        <div
          v-for="(context, index) in modelValue"
          :key="index"
      ...
      <div v-else style="color: #a1a5b7;font-weight: 500;font-size: 1.1rem;overflow: hidden;white-space: nowrap;text-overflow: ellipsis;" v-html="placeholder || 'Pilih Data'"></div>
      
      <div class="d-flex" style="margin-left: 10px">
        <div v-if="hasValue(modelValue) && clearable && !disabled" class="select-multiple-remove-icon" @click="handleRemoveValue">
          <svg width="14px" height="14px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path 
              fill-rule="evenodd" 
              clip-rule="evenodd" 
        ...
        <div>
          <svg height="13px" width="13px" viewBox="0 0 26 15">
            <polygon fill="#231F20" points="23.303,-0.002 12.467,10.834 1.63,-0.002 -0.454,2.082 12.467,15.002 14.551,12.918 25.387,2.082" />
          </svg>
        </div>
      </div>
    </div>
    ...
    <div v-if="isDropdownOpen" class="select-multiple-option">
      <div v-if="searchable" class="select-multiple-option-search" style="border-bottom: 1px solid #e5e5e5;display: flex;align-items: center;">
        <div>
        ...
        <input
          type="text"
          placeholder="Search...." style="border: 0 !important;outline: none;box-shadow: unset;"
...
.vue-select-multiple-energeek .select-multiple-option-list a:hover, .vue-select-multiple-energeek .select-multiple-option-list a.active{
